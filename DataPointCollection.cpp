...
namespace MicrosoftResearch { namespace Cambridge { namespace Sherwood
{
	...
	std::auto_ptr<DataPointCollection> DataPointCollection::LoadImages(
		std::string path, 
		...
		int patch_size)
	{
		...
		// Flags to show if the data loaded should have targets/classes or both.
		bool bHasTargetValues = (descriptor == DataDescriptor::TargetValues)||(descriptor == DataDescriptor::Both);
		bool bHasClassLabels =  (descriptor == DataDescriptor::Classes)||(descriptor == DataDescriptor::Both);
		...
		result->pixelLabels_ = IPUtils::generateDepthBinMap(true, 5, max);
		...
		for (int i = first; i <= last;i++)
		{
			...
			std::cout << "loading " << ir_path << endl;
			std::cout << "loading " << depth_path << endl;
			...
			cv::Mat patch;
			cv::Size ret_patch_size;
			for (int r = 0; r < ir_size.height; r++)
			{
				for (int c = 0; c < ir_size.width; c++)
				{
